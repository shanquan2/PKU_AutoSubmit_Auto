...
class Pusher():
    def push(self, method, method_args: list or tuple, title, text):
        if method == 'wechat':
            if len(method_args) == 1:
                appid = method_args[0]
                self.wechat_push(title, text, appid)
            else:
                return 'push:invalid args input for method wechat'

    def wechat_push(self, title, text, appid):
        requests.post(f'https://sc.ftqq.com/{appid}.send', data={'text': title, 'desp': text})
